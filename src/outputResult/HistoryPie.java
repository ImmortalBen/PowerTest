...
public class HistoryPie extends JFrame {
	// 初始数据
	private ArrayList<Double> data=Calculator.HistoryPowerNum;
	//private ArrayList<String> method=Caculator.method;
	//private double data[] = { 100., 150, 113., 127., 299.234, 120., 93., 123.,
	//		127., 199., 157., 189. };
	private String percent[] = new String[Calculator.HistoryPowerNum.size()]; // 每个数据所占百分比
	private int radian[] = new int[Calculator.HistoryPowerNum.size()]; // 每个数据所对应的弧度数
	...
	public void PaintBackground(Graphics g) {
		...
		for (int i = 0; i < data.size(); i++) {
			...
			g2.drawString(String.valueOf(data.get(i)) +String.valueOf(ConsoleResult.GetMathod(" "+Calculator.HistoryMethod.get(i)))+ "   (" + percent[i] + ")",
	...
	public static void main(String[] arsg) {
		try {
			HistoryPie cake = new HistoryPie();
		} catch (Exception exe) {
		}
	}
}